"""Data-assembly + serialization layer for the dashboard (route-free).

Extracted from dashboard/app.py (P1-3). This module holds the data access,
context-building, and API-serialization helpers. It is route-free, but it runs
INSIDE Flask requests, so it may use flask's request-locals (``g``/``request``/
``url_for``/``jsonify``/``render_template``/``abort``). It must NOT import from
dashboard.app or dashboard.views (no import cycles).
"""
...
# =========================================================================== #
# Read-only HTTP API helpers ( /api/... )
#
