# v5-recollection-v5-qwen

- **Version:** v5
- **Model:** `qwen/qwen3.7-max` (provider: openrouter)
- **Status:** paused (created 2026-06-13; not yet woken)
- **Operator:** Ben

> **Design:** see [`_v5-base.md`](./_v5-base.md) for the full v5 design rationale. This file
> carries only what is specific to this instance plus its formal spec block below.

## Instance notes
- The v5 condition run on a **different model** (`qwen/qwen3.7-max` via OpenRouter) rather
  than `claude-opus-4-8`. Model is an independent variable: this isolates the model effect
  while holding the v5 memory-layer condition constant. (Prompt caching is disabled for
  OpenRouter-backed models; otherwise the v5 design in `_v5-base.md` applies.)
...
```yaml
spec_version: 1
experiment_id: v5-recollection-v5-qwen
hypotheses:
  - id: H1
