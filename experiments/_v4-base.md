# v4 — `continuous` (shared design base)

> **This is the shared design rationale for every v4 instance.** It is not itself an
> instance doc — `research/spec.py` never loads it (it only reads `experiments/<instance_id>.md`).
> The per-instance files — `v4-continuous.md`, `v4-continuous-8a.md`,
> `v4-continuous-qwen-max.md` — each carry their own `{id, model, status, dates,
> instance-specific notes}` plus their **own** `## Specification (formal)` ```yaml block
> (the panel's ground truth; its hash gates the coding scheme) and point back here for the
> design. Edit the design once, here; keep each instance's yaml spec block untouched.
